...
class ShowcaseController (val showcaseService: ShowcaseService){
    ...
    fun delete(@PathVariable("id")id : Int):BaseResponse<String>{
        ...
        return BaseResponse(
            code = 200,
            status = "List Showcase Success",
            data = "delete showcase Success"
        )
    }
    @PutMapping(
        value = ["api/showcase/{id}"],
    )
    fun put(@PathVariable("id")id: Int , updateShowcaseRequest: UpdateShowcaseRequest): BaseResponse<ShowcaseResponse>{
        val showcaseResponse = showcaseService.put(id, updateShowcaseRequest)
        return BaseResponse(
            code = 200,
            status = "Update Showcase Succes",
            data = showcaseResponse
        )
    }
}